...
function verifyAcres() {
    ...
    try {
        ...
        messageHeadElement.innerHTML = ""; // remove any former reccommendation heading
...
function verifyCrops() {
    // creates a try/catch statement to throw a message if none of the crop boxes are checked which is checked for by running through each input element in the fieldset to see if it is checked and if so it ends the loop checking for checked boxes
    try {
        for (var i = 0; i < 7; i++) {
        ...
        //removed error here which was the word Message which caused an error
    }
}
